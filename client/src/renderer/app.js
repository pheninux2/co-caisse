...
class CocaisseApp {
  ...
  async loadFloorPlan() {
    try {
      const [layoutRes, statusRes, bgRes, drawRes] = await Promise.all([
        this.apiFetch(`${API_URL}/tables/layout`),
        this.apiFetch(`${API_URL}/tables/status`),
        this.apiFetch(`${API_URL}/tables/background`),
        this.apiFetch(`${API_URL}/tables/drawing`),
      ]);
  ...
  async uploadFloorBackground(input) {
    ...
    try {
      ...
      // Recharger pour afficher l'image
      const bgRes  = await this.apiFetch(`${API_URL}/tables/background`);
      const bgData = await bgRes.json();
  ...
  async removeFloorBackground() {
    ...
    try {
      const res = await this.apiFetch(`${API_URL}/tables/background`, { method: 'DELETE' });
  ...
  /** Outil actif : 'select' | 'table' | 'wall' | 'door' | 'window' | 'zone' */
  _currentDrawTool = 'select';
  _drawElements    = [];      // éléments sauvegardés en base
  _selectedDrawEl  = null;    // id de l'élément sélectionné
  _drawing         = false;   // tracé en cours
  _drawStart       = null;    // { x, y } point de départ
  ...
  async saveDrawing() {
    try {
      const res  = await this.apiFetch(`${API_URL}/tables/drawing`, {
        method: 'PUT',
        body:   JSON.stringify({ elements: this._drawElements }),
      });
      const data = await res.json();
    ...
    el.addEventListener('pointerup', async () => {
      ...
      try {
        await this.apiFetch(`${API_URL}/tables/${tableId}`, {
          method: 'PUT', body: JSON.stringify({ x: newX, y: newY }),
        });
  ...
  async saveTable() {
    ...
    try {
      ...
      const res = await this.apiFetch(url, { method: id ? 'PUT' : 'POST', body: JSON.stringify(body) });
      const data = await res.json();
...
CocaisseApp.prototype.editOrder = async function(orderId) {
  try {
    ...
    // Charger les tables dans le select, puis sélectionner la bonne
    await this.refreshTableSelect();
    const tableSelect = document.getElementById('orderTableNumber');
