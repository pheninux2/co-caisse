/**
 * Co-Caisse — SettingsService
 */
import { v4 as uuidv4 } from 'uuid';

const DEFAULTS = {
  company_name:                '',
  company_address:             '',
  company_phone:               '',
  company_email:               '',
  tax_number:                  '',
  currency:                    'EUR',
  default_tax_rate:            20,
  receipt_header:              '',
  receipt_footer:              '',
  alert_enabled:               1,
  alert_sound_enabled:         1,
  alert_draft_minutes:         15,
  alert_validated_minutes:     10,
  alert_kitchen_minutes:       20,
  alert_ready_minutes:         5,
  alert_served_minutes:        30,
  alert_remind_after_dismiss:  10,
  country:                     'FR',
};

export const SettingsService = {

  async get(db) {
    const row = await db.get('SELECT * FROM settings LIMIT 1');
    return row ?? DEFAULTS;
  },

  async upsert(db, data) {
    const {
      company_name,
      company_address,
      company_phone,
      company_email,
      tax_number,
      currency                 = 'EUR',
      default_tax_rate         = 20,
      receipt_header,
      receipt_footer,
      alert_enabled            = 1,
      alert_sound_enabled      = 1,
      alert_draft_minutes      = 15,
      alert_validated_minutes  = 10,
      alert_kitchen_minutes    = 20,
      alert_ready_minutes      = 5,
      alert_served_minutes     = 30,
      alert_remind_after_dismiss = 10,
      fiscal_chain_enabled     = 0,
      fiscal_day_start_hour    = 6,
      agec_enabled             = 1,
      rgpd_retention_months    = 120,
      rgpd_logs_retention_months = 12,
      country                  = 'FR',
    } = data;

    const rgpdMonths     = Math.max(parseInt(rgpd_retention_months)      || 1, 1);
    const rgpdLogsMonths = Math.max(parseInt(rgpd_logs_retention_months) || 1, 1);

    const existing = await db.get('SELECT id FROM settings LIMIT 1');

    if (existing) {
      await db.run(
        `UPDATE settings SET
          company_name = ?,
          company_address = ?,
          company_phone = ?,
          company_email = ?,
          tax_number = ?,
          currency = ?,
          default_tax_rate = ?,
          receipt_header = ?,
          receipt_footer = ?,
          alert_enabled = ?,
          alert_sound_enabled = ?,
          alert_draft_minutes = ?,
          alert_validated_minutes = ?,
          alert_kitchen_minutes = ?,
          alert_ready_minutes = ?,
          alert_served_minutes = ?,
          alert_remind_after_dismiss = ?,
          fiscal_chain_enabled = ?,
          fiscal_day_start_hour = ?,
          agec_enabled = ?,
          rgpd_retention_months = ?,
          rgpd_logs_retention_months = ?,
          country = ?,
          updated_at = CURRENT_TIMESTAMP
        WHERE id = ?`,
        [
          company_name, company_address, company_phone, company_email, tax_number,
          currency, default_tax_rate, receipt_header, receipt_footer,
          alert_enabled, alert_sound_enabled,
          alert_draft_minutes, alert_validated_minutes, alert_kitchen_minutes,
          alert_ready_minutes, alert_served_minutes, alert_remind_after_dismiss,
          fiscal_chain_enabled, fiscal_day_start_hour, agec_enabled,
          rgpdMonths, rgpdLogsMonths, country,
          existing.id,
        ]
      );
    } else {
      await db.run(
        `INSERT INTO settings (
          id, company_name, company_address, company_phone, company_email, tax_number,
          currency, default_tax_rate, receipt_header, receipt_footer,
          alert_enabled, alert_sound_enabled,
          alert_draft_minutes, alert_validated_minutes, alert_kitchen_minutes,
          alert_ready_minutes, alert_served_minutes, alert_remind_after_dismiss,
          fiscal_chain_enabled, fiscal_day_start_hour, agec_enabled,
          rgpd_retention_months, rgpd_logs_retention_months, country
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          uuidv4(), company_name, company_address, company_phone, company_email, tax_number,
          currency, default_tax_rate, receipt_header, receipt_footer,
          alert_enabled, alert_sound_enabled,
          alert_draft_minutes, alert_validated_minutes, alert_kitchen_minutes,
          alert_ready_minutes, alert_served_minutes, alert_remind_after_dismiss,
          fiscal_chain_enabled, fiscal_day_start_hour, agec_enabled,
          rgpdMonths, rgpdLogsMonths, country,
        ]
      );
    }

    return db.get('SELECT * FROM settings LIMIT 1');
  },

};
